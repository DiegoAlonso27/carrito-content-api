 ...
 *
 * Uso:
 *   npx tsx scripts/migrate/import-cache.ts [--file ruta] [--dry-run] [--verify] [--force]
 *
 ...
 * - --dry-run: valida y reporta sin escribir.
 * - --verify: tras importar, compara MongoDB contra el archivo (exit 1 si difiere).
 *
...
const { cache, errors } = validateCache(raw);
if (cache === null) {
  console.error(`El archivo ${args.file} no cumple el contrato:`);
  for (const e of errors) console.error(`  - ${e}`);
  process.exit(1);
}

// Gate de sanitización del HTML embebido (answerHtml/bodyHtml).
const changes = checkHtmlSanitization(cache);
if (changes.length > 0) {
